...
#else
    const char* pszModule = "Offerings";
#endif
    if (pex)
        return strprintf(
...
{
    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Offerings
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Offerings.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No Offerings.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override Offerings.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Offeringsd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Offerings will not work properly.");
